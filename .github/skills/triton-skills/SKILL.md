...
Triton 源码位于本项目下的 `.github/skills/triton-skills/repos/triton/`。

**TRITON_REPO**: 下文用 `TRITON_REPO` 指代该目录，实际使用时替换为项目根目录的绝对路径加上 `.github/skills/triton-skills/repos/triton/`。
...
先确定 TRITON_REPO 的实际绝对路径（项目根目录 + `.github/skills/triton-skills/repos/triton/`），然后用绝对路径搜索。

### Triton API 用法

```bash
# 设置路径变量（替换为实际路径）
TRITON_REPO="$(git rev-parse --show-toplevel)/.github/skills/triton-skills/repos/triton"
...
- Quick Reference: 见同目录下的 `quick-reference.md`
