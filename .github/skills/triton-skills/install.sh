...
#
# 用法:
#   bash .github/skills/triton-skills/install.sh [--full]
#
...
if [ "$1" = "-h" ] || [ "$1" = "--help" ]; then
    echo "用法: bash .github/skills/triton-skills/install.sh [--full]"
    echo ""
    echo "安装 Triton 源码到 .github/skills/triton-skills/repos/triton/"
    echo "供 AI 编码助手检索 Triton/Gluon 教程和参考实现。"
    echo ""
    echo "选项:"
    echo "  --full  拉取完整仓库（depth=1），默认使用 sparse checkout 只拉关键目录"
    exit 0
fi
...
check "$SCRIPT_DIR/SKILL.md" "SKILL.md"
check "$SCRIPT_DIR/quick-reference.md" "quick-reference.md"
